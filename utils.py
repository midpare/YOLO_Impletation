...
def get_label(xml_file_path, classes):
  ...
  # def setLabel(obj): 
    # class_index = np.where(classes == obj['name'].lower())[0]
    # label = class_index
  ...
  try:
    for obj in xml_file['annotation']['object']:
      # setLabel(obj)
      class_index = np.where(classes == obj['name'].lower().encode())[0][0]
      label = class_index
  except TypeError as e:
    obj = xml_file["annotation"]["object"]
    class_index = np.where(classes == obj['name'].lower().encode())[0][0]
    label = class_index
  return label

def get_data(xml_file_list, classes):
  image_path_list = []
  labels = []
  path = xml_file_list[0]
  for path in xml_file_list:
    f = open(path)
    xml_file = xmltodict.parse(f.read())
    ...
    image_path = "./VOC2012/JPEGImages/" + xml_file["annotation"]["filename"]
